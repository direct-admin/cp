...
<div id="page-wrapper">
            <div class="row">
                <div class="col-lg-12">
                    ...
                    <h1 class="page-header"> <?php echo EDIT_SRV_RECORDS . $dnsmanager['domain_name']; ?></h1>
                </div>
                <!-- /.col-lg-12 -->
            </div>
            <!-- /.row -->
  ...
  <div class="panel-heading">
     ...
     <?php echo EDIT_SRV_RECORDS . $dnsmanager['domain_name']; ?>
...
array("fieldname" => "dns_priority","fielddesc" => MX_PRIORITY,"fieldtype" => "text","fieldplaceholder" =>MX_PRIORITY_PLACEHOLDER,"fieldvalue" =>"","data-type" => "","data-required" => "true","validator" => ""),

array("fieldname" => "dns_weight","fielddesc" => SRV_WEIGHT,"fieldtype" => "text","fieldplaceholder" =>SRV_WEIGHT_PLACEHOLDER,"fieldvalue" =>"","data-type" => "","data-required" => "true","validator" => ""),

array("fieldname" => "dns_port","fielddesc" => SRV_PORT,"fieldtype" => "text","fieldplaceholder" =>SRV_PORT_PLACEHOLDER,"fieldvalue" =>"","data-type" => "","data-required" => "true","validator" => ""),
...
<!--  Showing A Records below -->
  ...
  <div class="panel-heading">
     ...
     <?php echo SRV_DNS_RECORDS . $dnsmanager['domain_name']; ?>
...
$numrows = $db->fetchOne('Select count(*) as nums from dns where dns_type="SRV" and dnsid= :dnsid', array(":dnsid"=>$global->clean_input($_GET['id'])));
$result = $db->fetchAll("Select * from dns where dns_type='SRV' and dnsid= :dnsid  order by dnssetid asc LIMIT ".$global->clean_pager($offset).", ".$global->clean_pager($rowsPerPage),array(":dnsid"=>$global->clean_input($_GET['id']))); 
...
$crud->AddHeaders(array(A_HOST_NAME,A_TTL,MX_PRIORITY,SRV_WEIGHT,SRV_PORT,A_TARGET,ACTIONS));

$crud->Table_Columns(array("dns_host","dns_ttl","dns_priority","dns_weight","dns_port","dns_target","action"));

$crud->AddActionLinks(array(array("linkdesc" =>"Delete", "linkfile" => "delete.php", "linkparam" =>"dnssetid", "linkalias" => "dnsid", "linkcss" => "btn btn-danger", "linkicon" => "<i class='fa fa-trash-o'></i>", "target"=>"_self", "moreparams"=>"&type=13&id=".$global->clean_input($_GET['id']) )));
