...
//			}
	...
	private function _query($sql = null, $params=null){
		...
		if(!empty($sql)){
			...
			if(!$statement) {
				...
				throw new PDOException("Driver Error[{$errorInfo[0]}]:{$errorInfo[2]}, driver error code is {$errorInfo[1]}");
			}
	...
	public function execute($sql = null, $params = null){
		...
		if(!empty($sql)){
			try{
				...
				echo $this->_exceptionOutput($e, "Something went wrong trying to executing the sql statement");
			}
		}
	...
	private function _areUpdateParametersValid($table = null, $array = null, $value= null, $field= null) {
		...
		return (!empty($table) && !empty($array) && !empty($field) && !empty($value));
	...
	private function _areDeleteParametersValid($table = null, $value= null, $field= null) {
		...
		return (!empty($table) && !empty($value) && !empty($field));
	...
	public function GetOne($table = null, $value= null, $field= 'id') {
